...
(function() {
    ...
    function validateConfig(config) {
        ...
        if (missingKeys.length > 0) {
            ...
            throw new Error(`Configuration Error: Missing keys - ${missingKeys.join(', ')}`);
        }
        ...
        const invalidUrls = requiredKeys.filter(key => !isValidUrl(config[key]));
        
        if (invalidUrls.length > 0) {
            console.error('Invalid URLs:', invalidUrls);
            throw new Error(`Configuration Error: Invalid URLs - ${invalidUrls.join(', ')}`);
        }
    }
        ...
        if (document.getElementById('loadingScreen')) {
            document.getElementById('loadingScreen').innerHTML = `
                <div class="loading-error">
                    ...
                    <p>${error.message}</p>
                    <p>Please contact support or check your configuration.</p>
                </div>
            `;
        }
    }
